...
# ==== Pathway Annotation ====
gs1_lst = readRDS("/net/hsphfs1/srv/export/hsphfs1/share_root/hide_lab/PCxN/data/gobp_gs.RDS")
gs2_lst = readRDS("/net/hsphfs1/srv/export/hsphfs1/share_root/hide_lab/PCxN/data/cp_gs.RDS")
...
ProcessElement <- function(ic){
  ...
  if(over_check){
    ...
    if(uCheck){
      ...
      tmp = data.frame(Pathway.A=names(gs1_lst)[i],Pathway.B=names(gs2_lst)[j])
      tmp = c(tmp,ShrinkPCor(summaryAu,summaryBu,summaryAB,method="spearman"))
      # overlap coefficient
      tmp$Overlap.Coeff= OverlapCoefficient(gs1_lst[[i]],gs2_lst[[j]])
    }
  }else{
    # no overlap
    # get pathway summaries
...
input = 1:(n1*n2)
pb = txtProgressBar(min=0,max=n1*n2,style=3,initial=0)
cat("\n")
...
# save results
saveRDS(res,paste0("/net/hsphfs1/srv/export/hsphfs1/share_root/hide_lab/PCxN/output/merge/",gse,"_cp_gobp_pathcor.RDS"))
rm(list=ls())
