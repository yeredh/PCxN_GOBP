# gse_annot = readRDS("Hide Lab/PCxN/data/GSE_annotation.RDS")
...
setwd("/net/hsphfs1/srv/export/hsphfs1/share_root/hide_lab/PCxN/output/merge")
rds_files = sort(list.files(pattern="RDS"))
gse_done = sort(sapply(rds_files,function(x){gsub("(GSE[0-9]*)_cp_gobp_pathcor.*","\\1",x)}))
names(gse_done) = NULL
